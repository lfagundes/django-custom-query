...
class SingleParserTest(BaseTest):
    ...
    def test_tilda_operator_number(self):
        self.assertEquals(self.parse("numfield ~ 0.1"),
    ...
    def test_in_operator_with_numerical_field(self):
        self.assertEquals(self.parse("numfield IN (1, 2, 3, 4)"),
                          Q(numfield__in=(1, 2, 3, 4)))

    def test_not_in_operator(self):
        self.assertEquals(self.parse("numfield NOT IN (1, 2, 3, 4)"), ~Q(
            numfield__in=(1, 2, 3, 4)))

    def test_in_operator_with_string_field(self):
        self.assertEquals(self.parse("charfield IN ('one', 'two', 'three')"), Q(
...
class AnnotationTest(BaseTest):
    ...
    def test_annotated_field(self):
        self.assertEquals(self.parse('full_name="foo bar"'),
                          Q(full_name="foo bar"))
...
class MathOperatorTest(BaseTest):
    ...
    def test_plus_gt(self):
        self.assertEquals(self.parse('numfield + numfield2 > 1'),
                          Q(numfield__gt=1-F('numfield2')))

    def test_plus_gte(self):
        self.assertEquals(self.parse('numfield + numfield2 >= 2.0'),
                          Q(numfield__gte=2.0-F('numfield2')))

    def test_plus_lt(self):
        self.assertEquals(self.parse('numfield + numfield2 < -2'),
                          Q(numfield__lt=-2-F('numfield2')))

    def test_plus_lte(self):
        self.assertEquals(self.parse('numfield + numfield2 <= -4.5'),
    ...
    def test_plus_not(self):
        self.assertEquals(self.parse('numfield + numfield2 NOT 1'),
                          ~Q(numfield=1-F('numfield2')))
    ...
    def test_multiplication_lt(self):
        self.assertEquals(self.parse('numfield * numfield2 < -3'),
                          Q(numfield__lt=-3 / F('numfield2')))

    def test_multiplication_lte(self):
        self.assertEquals(self.parse('numfield * numfield2 <= -6.7'),
...
class ParenthesisTest(BaseTest):
    ...
    def test_one_parenthesis_mathexpression(self):
        self.assertEquals(self.parse('numfield=1 or (numfield > 5 and numfield - numfield2 < 10)'),
...
class BetweenTest(BaseTest):
    ...
    def test_between_number(self):
        self.assertEquals(self.parse('numfield between 1 and 5'),
                          Q(numfield__gte=1) & Q(numfield__lte=5))

    def test_between_number_mathexpression(self):
        self.assertEquals(self.parse('numfield - numfield2 between 1 and 5'),
...
class DateFormatTest(TestCase):
    ...
    def test_date_without_format(self):
        ...
        self.assertEquals(parser.parse('datefield="2018-12-13"'),
                          Q(datefield=date(2018, 12, 13)))

    def test_date_with_format(self):
        parser = Parser(TestModel, date_format='%d/%m/%Y')
        self.assertEquals(parser.parse('datefield="13/12/2018"'),
                          Q(datefield=date(2018, 12, 13)))
        self.assertEquals(parser.parse('datefield=13/12/2018'),
                          Q(datefield=date(2018, 12, 13)))
